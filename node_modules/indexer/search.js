...
function Search(context, s1Id, s2Id) {
    ...
    this.seenTable = new Table(this.identity.hashCount());
    this.nodeCount = 0;
...
Search.prototype.nextIteration = function(depth, node) {
    ...
    if (pruneValue == 15) {
        ...
        if (this.seenTable.getValue(node.state.hash()) == depth) {
            return;
        } else {
            this.seenTable.setValue(node.state.hash(), depth);
        }
    }
...
Search.prototype.initiateBacksearch = function(depth) {
    this.seenTable = null;
}

Search.prototype.iterateBacksearch = function(depth, callback) {
    for (var i = 0; i < this.context.hashCount(); i++) {
        ...
        if (i % 100000 == 0 && callback) {
            callback(100 * i / this.context.hashCount());
        }
    }
}
