...
CoMap.prototype.moveFace = function(face) {
    ...
    for (var i = 0; i < 4; i++) {
        ...
        if (target[slot] != transformation[2]) {
            if (transformation[2] == 2) {
                target[slot] ^= 1;
            } else if (transformation[2] == 1) {
...
CoMap.prototype.rotateY = function(yPower) {
    return this.move(new Move(2, yPower)).move(new Move(3, yPower));
}
...
CoMap.prototype.flipLR = function(flag) {
    ...
    for (var i = 0; i < 4; i++) {
        ...
        target[i] = (target[i] == 2 ? 0 : 2);
        target[i | 4] = (target[i | 4] == 2 ? 0 : 2);
    }
    return new CoMap(target);
}
...
exports.generate = function(prefix) {
    if (!prefix) return exports.generateAllMaps([]);
    if (prefix.length == 7) {
    ...
    for (var i = 0; i < 3; i++) {
        ...
        var values = exports.generateAllStates(next);
